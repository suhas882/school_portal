...
// ==========================================
// DASHBOARD STATS (Financial integration added)
// ==========================================
router.get('/stats', async (req, res) => {
  try {
    ...
    const homeworkCount = await query.get(
      'SELECT COUNT(*) as count FROM homework WHERE DATE(created_at) = ?',
      [today]
    );
    ...
    res.json({
      ...
      homeworkToday: homeworkCount.count,
      totalIncome,
      totalExpenses,
      netBalance,
      recentActivities
...
router.put('/classes/:id', async (req, res) => {
  ...
  try {
    const oldClass = await query.get('SELECT name FROM classes WHERE id = ?', [id]);
    await query.run(
      'UPDATE classes SET name = ?, day_scholar_fee = ?, hostel_student_fee = ? WHERE id = ?',
      [name.trim(), dayFee, hostelFee, id]
    );

    // Sync fees for all students currently registered under this class
    // 1. Sync Day Scholars
    const dayScholars = await query.all('SELECT id, concession FROM students WHERE class_id = ? AND type = "Day Scholar"', [id]);
    for (const student of dayScholars) {
      const paymentsSum = await query.get('SELECT SUM(amount) as total FROM payments WHERE student_id = ?', [student.id]);
      const paid = paymentsSum.total || 0;
      const feeAfter = dayFee - student.concession;
      const remaining = Math.max(0, feeAfter - paid);
      const status = remaining <= 0 ? 'Paid' : 'Pending';
      await query.run(
        'UPDATE students SET fee_amount = ?, fee_after_concession = ?, remaining_balance = ?, payment_status = ? WHERE id = ?',
        [dayFee, feeAfter, remaining, status, student.id]
      );
    }
    // 2. Sync Hostel Students
    const hostelStudents = await query.all('SELECT id, concession FROM students WHERE class_id = ? AND type = "Hostel Student"', [id]);
...
router.put('/classes/:id/fees', async (req, res) => {
  ...
  try {
    await query.run(
      'UPDATE classes SET day_scholar_fee = ?, hostel_student_fee = ? WHERE id = ?',
      [dayFee, hostelFee, id]
    );
    ...
    const cls = await query.get('SELECT name FROM classes WHERE id = ?', [id]);
...
router.delete('/classes/:id', async (req, res) => {
  ...
  try {
    ...
    const activeStudents = await query.get('SELECT COUNT(*) as count FROM students WHERE class_id = ?', [id]);
...
router.get('/sections/by-class/:class_id', async (req, res) => {
  try {
    const sections = await query.all('SELECT * FROM sections WHERE class_id = ? ORDER BY name', [req.params.class_id]);
    res.json(sections);
  } catch (error) {
...
router.post('/sections', async (req, res) => {
  ...
  try {
    const result = await query.run('INSERT INTO sections (name, class_id) VALUES (?, ?)', [name.trim().toUpperCase(), class_id]);
    const cls = await query.get('SELECT name FROM classes WHERE id = ?', [class_id]);
...
router.delete('/sections/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const sec = await query.get('SELECT * FROM sections WHERE id = ?', [id]);
    if (!sec) return res.status(404).json({ message: 'Section not found' });

    const activeStudents = await query.get('SELECT COUNT(*) as count FROM students WHERE section_id = ?', [id]);
    if (activeStudents.count > 0) {
      return res.status(400).json({ message: 'Cannot delete section containing students' });
    }

    await query.run('DELETE FROM sections WHERE id = ?', [id]);
    const cls = await query.get('SELECT name FROM classes WHERE id = ?', [sec.class_id]);
...
router.get('/subjects/by-class/:class_id', async (req, res) => {
  try {
    const subjects = await query.all('SELECT * FROM subjects WHERE class_id = ? ORDER BY name', [req.params.class_id]);
    res.json(subjects);
  } catch (error) {
...
router.post('/subjects', async (req, res) => {
  ...
  try {
    const result = await query.run('INSERT INTO subjects (name, class_id) VALUES (?, ?)', [name.trim(), class_id]);
    const cls = await query.get('SELECT name FROM classes WHERE id = ?', [class_id]);
...
router.delete('/subjects/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const sub = await query.get('SELECT * FROM subjects WHERE id = ?', [id]);
    if (!sub) return res.status(404).json({ message: 'Subject not found' });

    await query.run('DELETE FROM subjects WHERE id = ?', [id]);
    const cls = await query.get('SELECT name FROM classes WHERE id = ?', [sub.class_id]);
...
// ==========================================
// STUDENTS (Updated with tuition logic)
// ==========================================
router.get('/students', async (req, res) => {
  ...
  if (search && search.trim()) {
    querySql += ' AND (s.name LIKE ? OR s.roll_number LIKE ?)';
    params.push(`%${search.trim()}%`, `%${search.trim()}%`);
  }

  if (class_id) {
    querySql += ' AND s.class_id = ?';
    params.push(class_id);
  }

  if (section_id) {
    querySql += ' AND s.section_id = ?';
  ...
  try {
    ...
    querySql += ' ORDER BY c.name, sec.name, s.name LIMIT ? OFFSET ?';
...
router.post('/students', async (req, res) => {
  ...
  try {
    ...
    const classData = await query.get('SELECT day_scholar_fee, hostel_student_fee FROM classes WHERE id = ?', [class_id]);
...
router.put('/students/:id', async (req, res) => {
  ...
  try {
    ...
    const paymentsSum = await query.get('SELECT SUM(amount) as total FROM payments WHERE student_id = ?', [id]);
    const paid = paymentsSum.total || 0;
    const remaining = Math.max(0, feeAfter - paid);
    const status = remaining <= 0 ? 'Paid' : 'Pending';
    ...
    await query.run(
      `UPDATE students 
       SET name = ?, roll_number = ?, class_id = ?, section_id = ?, parent_id = ?, type = ?, fee_amount = ?, concession = ?, fee_after_concession = ?, remaining_balance = ?, payment_status = ? 
       WHERE id = ?`,
...
router.delete('/students/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const student = await query.get('SELECT name FROM students WHERE id = ?', [id]);
...
router.get('/students/:id/payments', async (req, res) => {
  try {
    const payments = await query.all('SELECT * FROM payments WHERE student_id = ? ORDER BY date DESC, created_at DESC', [req.params.id]);
    res.json(payments);
  } catch (error) {
...
router.post('/students/:id/payments', async (req, res) => {
  ...
  try {
    const student = await query.get('SELECT name, remaining_balance, fee_after_concession FROM students WHERE id = ?', [id]);
    if (!student) return res.status(404).json({ message: 'Student profile not found' });

    if (parsedAmount > student.remaining_balance) {
      return res.status(400).json({ message: `Payment exceeds student's remaining balance of ₹${student.remaining_balance}` });
    }

    // Insert payment record
    const result = await query.run('INSERT INTO payments (student_id, amount, date) VALUES (?, ?, ?)', [id, parsedAmount, date]);

    // Recalculate student balance
    const nextBalance = Math.max(0, student.remaining_balance - parsedAmount);
...
router.delete('/expenses/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const exp = await query.get('SELECT item_name, amount FROM expenses WHERE id = ?', [id]);
...
router.post('/parents', async (req, res) => {
  ...
  try {
    ...
    const result = await query.run(
      'INSERT INTO parents (username, email, password_hash, phone) VALUES (?, ?, ?, ?)',
...
router.put('/parents/:id', async (req, res) => {
  ...
  try {
    await query.run(
      'UPDATE parents SET username = ?, email = ?, phone = ? WHERE id = ?',
...
router.delete('/parents/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const parent = await query.get('SELECT username FROM parents WHERE id = ?', [id]);
    if (!parent) return res.status(404).json({ message: 'Parent not found' });

    await query.run('UPDATE students SET parent_id = NULL WHERE parent_id = ?', [id]);
    await query.run('DELETE FROM parents WHERE id = ?', [id]);
...
router.post('/parents/:id/reset-password', async (req, res) => {
  ...
  try {
    ...
    await query.run('UPDATE parents SET password_hash = ? WHERE id = ?', [hash, id]);
...
router.put('/notices/:id', async (req, res) => {
  ...
  try {
    await query.run(
      'UPDATE notices SET title = ?, content = ?, audience = ? WHERE id = ?',
...
router.delete('/notices/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await query.run('DELETE FROM notices WHERE id = ?', [id]);
...
router.put('/school-info', async (req, res) => {
  ...
  try {
    await query.run(
      'UPDATE school_info SET name = ?, address = ?, phone = ?, email = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1',
